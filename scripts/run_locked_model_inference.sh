...
#base_models=("meta-llama/Llama-3.1-8B-Instruct" "mistralai/Mistral-7B-Instruct-v0.2")
base_models=("Qwen/Qwen3-0.6B" "Qwen/Qwen3-1.7B" "Qwen/Qwen3-4B" "Qwen/Qwen3-8B" "Qwen/Qwen3-14B" "Qwen/Qwen3-32B")
locking_methods=("dpo")
topics=("athelete")
temperatures=(0.0)
...
for model in "${base_models[@]}"; do
  model_abr="${model#*/}"
    for locking_method in "${locking_methods[@]}"; do
        for topic in "${topics[@]}"; do
        model_name="${model_abr}-locked-${topic}-${locking_method}"
        for temperature in "${temperatures[@]}"; do
            for dataset in "${datasets[@]}"; do
                echo "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX" 
                    echo "Performing inference on $dataset using $model_name with topic $topic at temperature $temperature"
                    python infer.py --model_name $huggingface_repo_namespace/$model_name --input_file $data_dir/$dataset/inference_input_default_train.csv --output_file $data_dir/$dataset/inference_input_${topic}_train_${model_name}_${temperature}_output.jsonl \
