...
@click.option("--huggingface_storage_repo_name", type=str, default=None, help="The name of the repo on the hub.")
@click.pass_context
def main(ctx, **input_parameters):
    ...
    if "huggingface_storage_repo_name" not in loaded_parameters:
        if input_parameters["huggingface_storage_repo_name"] is not None:
            loaded_parameters["huggingface_storage_repo_name"] = input_parameters["huggingface_storage_repo_name"]
        else:
            log_error("huggingface_storage_repo_name must be specified either in the config file or as a command line argument.", loaded_parameters)
    compute_secondary_parameters(loaded_parameters)
    loaded_parameters["huggingface_repo_name"] = loaded_parameters["huggingface_storage_repo_name"]
    api = HfApi()
